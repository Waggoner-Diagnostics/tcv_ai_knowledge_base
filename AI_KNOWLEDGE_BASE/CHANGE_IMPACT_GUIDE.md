...
| `Credits::addCreditsToUser()` | Called by admin grant, purchase (`BasePaymentProvider`), and **two** refund paths. It sets `credited_by = auth()->id()` — which is wrong for the invitation-cancel path already. **`ws-402` (merged 2026-09-07)** adds `original_source` to the payload on both refund paths |
