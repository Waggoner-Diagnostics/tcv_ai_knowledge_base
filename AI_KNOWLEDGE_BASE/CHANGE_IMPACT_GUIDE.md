...
| ☠️ **`config/legacy.php` or anything in `Support/Legacy/`** (`ws-459`, on `develop` 2026-09-18) | every patient read and write, the whole `migrate:*` family, and both blind-index lookups | `LEGACY_MASTER_KEY` keys **both** decryption and the md5 blind indexes. Change how a key resolves and either patients stop being findable by email (indexes built under a different key) or `blindIndex()` throws. Rotating it means rebuilding every index — `patients:rebuild-email-index --apply`. Never give it a default ([S-21](SECURITY.md#s-21--the-patient-blind-indexes-are-unsalted-md5-under-a-single-global-key) · [DATA_MIGRATION_CONTEXT](CONTEXT/DATA_MIGRATION_CONTEXT.md)) |
| ☠️ **`Patient::BLIND_INDEXES` or the `saving` hook** | every patient lookup by email or name | `syncBlindIndexes()` rebuilds from `$this->attributes`, so a model loaded with a narrowed `select()` has no `email` attribute — indistinguishable from a null email. It used to overwrite the hash with `NULL` and make the patient **permanently unfindable**; it now skips any index whose source column was not loaded. Preserve that check ([DATA_MIGRATION_CONTEXT](CONTEXT/DATA_MIGRATION_CONTEXT.md)) |
...
| A placeholder **added** to `catalogue()`, for either type | `TestInvitationMailer` must substitute it in the same commit, or the editor offers a token nothing fills and it reaches the patient as literal `{{…}}` text. Since `ws-401` (on `develop` since 2026-09-21, PR #252) the mailer renders `org_test_link` as well as `test_link`, so an org-only token now has a renderer to keep in sync: `organizationVariables()` for the four org values, the `$variables` map for everything else. `EmailTemplatePlaceholderValidationTest` asserts `known()` matches the mailer **per type** — that pair of tests is the guard rail ([INVITATION_CONTEXT](CONTEXT/INVITATION_CONTEXT.md)) |
| A **pass added to `TestInvitationMailer::send()`** | It goes **above** the `org_test_link` block, never below it. Since `ws-401` (on `develop` since 2026-09-21) that block fills four user-typed values, and it is placed last precisely so no later pass can act on them: put it before the generic `str_replace` and a name holding `{{verification_link}}` is substituted; before `EmailContent::linkify()` and a name holding `https://…` ships as a working anchor in an org-branded email. `e()` does not escape a URL scheme ([INVITATION_CONTEXT](CONTEXT/INVITATION_CONTEXT.md), [SECURITY](SECURITY.md)) |
...
| Anything that must hold server-side on the purchase path | ☠️ **Four handlers, not one**: `PaymentController::initializePayment()` *and* `confirmPayment()` (the live path — separate requests, and only `confirm` writes the grant), plus both legacy `StripePaymentController` halves, which no SPA code calls but which are still routed. `ws-480` originally put its 422 on the legacy `createPaymentIntent()` alone, so the rule did not bind where money moves. Put a refusal ahead of `DiscountCodeService::validate()` in `confirmPayment()`, or it writes discount audit rows for a checkout that never happened. ⚠️ **`ws-451` (unmerged) makes it five.** `PaymentController::completeFreeOrder()` writes a `SOURCE_PURCHASE` grant for $0 orders without touching Stripe ([BILLING_CONTEXT trap 10](CONTEXT/BILLING_CONTEXT.md#10-a-100-discount-code-cannot-go-through-stripe-ws-451-unmerged)) |
| Pricing (`price_details`) or `DiscountCodeService::calculate()` | `ws-451` (unmerged): `completeFreeOrder()` re-prices the order from `price_details` on the server and refuses it unless `final_amount` is 0. A change to either decides whether a code grants credits for free. Keep the SPA's `MIN_CHARGE_AMOUNT` in `PaymentForm.js` equal to the `amount => min:1` rule on `api/payment/*` |
...
| ☠️ **`patient_tests.is_email_invite` is three-state since `ws-459`** (nullable, `2026_09_16_000001`) | Backend writes `true`/`false`/`null`; **`null` means the legacy record never said** (pre-2020-01-30 rows). The SPA must test `=== false`, never `!is_email_invite`, or every old migrated test is tagged **In-Office** — an assertion no row in the old database supports. Two call sites today, `PatientTestList.js` and `ResultPage.js`; any third must match ([FRONTEND.md](FRONTEND.md#migrated-records-read-differently-from-native-ones-ws-459)) |
| ☠️ **Any id shown to a customer for a migrated record** | `tests.id` / `patient_tests.id` are fresh surrogate keys — the same test is a different number in each system (legacy `9` "Waggoner CCVT" is `17` here). Show `legacy_id` (or `test_plate_id` on the result screen) with a fallback to the native id, `null` meaning "created here". Backend adds `legacy_id` to `users`, `patients`, `credits`, `tests`, `patient_tests` and `transactions`; the SPA reads it in `PatientTestList.js` and `ResultPage.js` ([FRONTEND.md](FRONTEND.md#migrated-records-read-differently-from-native-ones-ws-459)) |
| ☠️ **Any `where()` against patient / answer / invitation PII** | Those columns are **ciphertext with a random IV** since `ws-459` — no SQL comparison can match. Exact lookup goes through the keyed-md5 blind indexes (`identification`, `*_name_ident`), substring search through `PatientNameSearch` in PHP. A `unique:` rule against one silently passes everything ([DATA_MIGRATION_CONTEXT](CONTEXT/DATA_MIGRATION_CONTEXT.md)) |
| `countries` table data / `DropdownValuesController::getCountriesWithStates()` — `fix/countries-list-update`, PR #265, on `develop` 2026-09-18 | Consumed with no shared caller by `TCV-Website`'s `DistributorSignupClient.jsx` and `AuthModal.jsx`, and by `TCV-Frontend`'s `loginSlice.js` `fetchCountries` (Register, Checkout, `NewUserModal`, `OrganisationModal`, Profile/Settings, Admin tables) — fix once on the backend, it reaches all three. Neither frontend sorts the list; order is whatever the endpoint returns. ☠️ `WorldSeeder` truncates and reseeds all ~250 rows from the `nnjeim/world` package on every run, so a hand-fix to already-seeded data (e.g. a retired country name) must live in the seeder itself, not only in a migration — otherwise a fresh `migrate --seed` / `migrate:fresh --seed` silently reverts it, with the migration already marked applied ([THIRD_PARTY.md](THIRD_PARTY.md#country-and-state-reference-data-nnjeimworld)) |
| ☠️ **`compliances` / `organization_types` rows, or the `insertOrIgnore` loops in `MigrateTcvUsersAndOrganizations::runPreMigrationSetup()`** (`ws-459`, on `develop` since 2026-09-21, PR #271) | every Add Organisation dropdown, and the Static IP validation rule | These two are the **only** lookup tables written by both a seeder and a `migrate:*` command, so they are the only ones that can gain a duplicate set per run. `insertOrIgnore` did nothing until `2026_09_21_000001` added the unique indexes — keep them. ☠️ **Never renumber these rows:** `validations.js` requires `static_ip` unless `compliance_id === "1"`, so the identity of row 1 is a frontend contract; dedupe must merge onto the **lowest** id, and must repoint `organizations` **before** deleting, since neither column has an FK ([DATA_MIGRATION_CONTEXT trap 8](CONTEXT/DATA_MIGRATION_CONTEXT.md) · [ORGANIZATION_CONTEXT trap 8](CONTEXT/ORGANIZATION_CONTEXT.md)) |
| ☠️ **`users.legacy_country` / `legacy_state` and `User::$hidden`** (`ws-459`, backend PR #282 + frontend PR #414, on `develop` 2026-09-22) | The backend hides both, so login and profile responses do not carry raw legacy text. The SPA's Users grid, `NewUserModal` and `OrganisationModal` read both off the payload anyway, so **as merged, those three fallbacks never render** (fix on branch `ws-459-legacy-location-visible` in both repos, unmerged 2026-09-23. The frontend half is `mapOrganisationToFormData()`, which also dropped the fields). Only the `UserController` / `OrganizationController` detail views show the value, via `User::displayCountry()/displayState()` + `FormatsLocationLabels`. Wiring the SPA up means `makeVisible()` on the admin payloads (`userWithType`, `edit`, organisation `index`'s `user`), never removing them from `$hidden`. Renaming a `countries` / `states` row also breaks `LegacyLocationResolver::COUNTRY_ALIASES` / `STATE_ALIASES`, which map to names, not ids ([DATA_MIGRATION_CONTEXT](CONTEXT/DATA_MIGRATION_CONTEXT.md#legacy-countrystate-resolution-ws-459-pr-282)) |
