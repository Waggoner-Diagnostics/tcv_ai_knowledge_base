...
| **F-041** | Payments | `api/payment/*` | `PaymentManager` · `StripeProvider` · `BasePaymentProvider` · ⭐ `ws-480` (**unmerged**): `initializePayment()` **and** `confirmPayment()` → **422** for an unlimited-credit account. Both, because only `confirm` writes the grant. First shipped on F-042 alone — [BILLING trap 9](CONTEXT/BILLING_CONTEXT.md#9--the-unlimited-purchase-refusal-is-on-the-deprecated-surface-ws-480) | `transactions`, `transaction_details`, `user_stripe_details`, `credits` | [BILLING](CONTEXT/BILLING_CONTEXT.md) |
| **F-042** | Legacy Stripe surface | `api/stripe/*` (`auth:sanctum` since 2026-09-07) | `StripePaymentController` · `StripeService` · ⭐ `ws-480` (**unmerged**): `createPaymentIntent` and `confirmPayment` → **422** for an unlimited-credit account. Kept although the SPA never calls this surface — it is still routed. ⚠️ `confirmACHPayment()` is not guarded and not routed | same | [BILLING](CONTEXT/BILLING_CONTEXT.md) |
...
| **F-081** | Reference data | `api/dropdown/*` · `GET api/countries-with-states` | `DropdownValuesController` · 🔧 `fix/countries-list-update` (**unmerged**, `ddbeccf6`): `orderBy('name')` added, plus a `WorldSeeder`-embedded fix for two ISO-retired country names (Swaziland/Macedonia) that survives a reseed — [THIRD_PARTY.md](THIRD_PARTY.md#country-and-state-reference-data-nnjeimworld) | `countries`, `states`, `compliances`, `privileges`, `organization_types`, `organization_settings_options` | — |
...
| **F-090** | 🚧 Legacy data migration & encryption at rest (`TCV-Backend@tcv_data_migration`, **unmerged**) | 26 `migrate:*` artisan commands (`migrate:all-master`, `migrate:tcv-all`, …) — no HTTP surface | `Support/Legacy/{LegacyCipher,LegacyEncrypter,PatientNameSearch}` · `Services/Migration/LegacyCipher` · `Casts/{LegacyEncrypted,LegacyEncryptedInteger,ResultJsonWithEncryptedPii}` · `Console/Commands/BaseMigrationCommand` · `config/legacy.php` | reads `OLD_DB_CONNECTION` `tcv_*`; writes `patients`, `patient_tests`, `testanswers`, `test_invitations`, `users`, `organizations`, `credits`, `transactions`, `discount_codes`, `price_details` | [DATA_MIGRATION](CONTEXT/DATA_MIGRATION_CONTEXT.md) |
...
| `POST api/stripe/create-payment-intent` (`API-090`) and the rest of `api/stripe/*` | routed and guarded, but **no SPA caller** — the portal buys credits through `api/payment/initialize` → `api/payment/confirm`. ☠️ `ws-480` put its unlimited-credit 422 here *first*, and only here; it now sits on both surfaces ([BILLING trap 9](CONTEXT/BILLING_CONTEXT.md#9--the-unlimited-purchase-refusal-is-on-the-deprecated-surface-ws-480)) |
