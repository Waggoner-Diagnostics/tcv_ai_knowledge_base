...
| | ✅ merged to `develop` (`4570912b`): discount breakdown on payment-success, `test.invitation_sent_bulk` removal, distributor-enquiry key, `test.started` Credits Used, FK names in update diffs, pricing rows off manual credit grants. 🚧 unmerged on `TCV-Backend@feat/audit-trail-user-panel-improvement-14-sep` + `TCV-Frontend@ui/audit-trail-improvements-11-sep-26`: impersonation attribution (new `impersonator_*` columns + backfill; first `audit_logs` schema change), plus `auth.account_locked` (catalog 67 → **68**) and date-only logging. See AUDIT_TRAIL_BACKEND_CONTEXT.md §14–§15. | | | | |
