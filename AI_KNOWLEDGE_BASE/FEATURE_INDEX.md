...
| **F-082** | Audit trail (on `develop` since 2026-09-09) | `GET api/audit-logs`, `api/audit-logs/people`, `api/audit-logs/{id}` (Super Admin only) | `AuditLogController` · `Audit\AuditService` (incl. `impersonatorFor()`) · `Audit\AuditEventCatalog` (**68 events** / 8 categories, asserted by `AuditEventCatalogTest`) · `Concerns\BuildsAuditDiffs` · ~78 `->log()` call sites across 22 files · `AppServiceProvider::accountLockedResponse()` | `audit_logs` (+5 `impersonator_*` columns, 2026-09-15) | [AUDIT_TRAIL_BACKEND](CONTEXT/AUDIT_TRAIL_BACKEND_CONTEXT.md), [AUDIT_TRAIL_FRONTEND](CONTEXT/AUDIT_TRAIL_FRONTEND_CONTEXT.md) |
