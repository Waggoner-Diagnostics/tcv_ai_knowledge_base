...
| **F-041** | Payments | `api/payment/*` | `PaymentManager` · `StripeProvider` · `BasePaymentProvider` · ☠️ `ws-480`'s unlimited-credit refusal is **not** here — it went to F-042, so this surface still sells credits to an unlimited holder | `transactions`, `transaction_details`, `user_stripe_details`, `credits` | [BILLING](CONTEXT/BILLING_CONTEXT.md) |
| **F-042** | Legacy Stripe surface | `api/stripe/*` (`auth:sanctum` since 2026-09-07) | `StripePaymentController` · `StripeService` · ⭐ `ws-480` (**unmerged**): `createPaymentIntent` → **422** for an unlimited-credit account, on the surface the SPA never calls | same | [BILLING](CONTEXT/BILLING_CONTEXT.md) |
...
| `POST api/stripe/create-payment-intent` (`API-090`) and the rest of `api/stripe/*` | routed and guarded, but **no SPA caller** — the portal buys credits through `api/payment/initialize` → `api/payment/confirm`. ☠️ `ws-480` put its unlimited-credit 422 here ([BILLING trap 9](CONTEXT/BILLING_CONTEXT.md#9--the-unlimited-purchase-refusal-is-on-the-deprecated-surface-ws-480)) |
