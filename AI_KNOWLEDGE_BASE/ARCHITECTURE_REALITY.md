...
| Layer | Count | Notes |
|---|---|---|
| Controllers | **36** | Thin-ish. Real logic mostly delegated to Services. |
| Services | **35** | **Where the business logic lives.** Includes an 11-class `Lms/` subtree, plus `TestInvitationMailer` (`ws-404`), plus `Audit/AuditService` + `Audit/AuditEventCatalog` (`feat/ui-audit-trail`). |
| Models | **41** | Eloquent, 70 declared relationships. |
