...
**Repo:** `TCV-Backend` (branch `develop` — confirmed checked out)
**Frontend contract:** `TCV-Frontend` branch `feat/ui-audit-trail` (5 commits ahead of `develop`, as of 2026-09-09 — this was "2 commits" when this doc was originally written), specified in [AUDIT_TRAIL_FRONTEND_CONTEXT.md](./AUDIT_TRAIL_FRONTEND_CONTEXT.md). Phases 1–3 built against fixtures; Phase 4 (swap three function bodies for axios calls) and the login/logout instrumentation described in SERVICES.md are both done as of this update.
