...
| **LMS completions never arrive** | `lms_delivery_queue` rows stuck at `pending` ⇒ **no queue worker** — the environment is not running `COMPOSE_PROFILES=workers` ([QUEUES.md](../QUEUES.md)). Check `GET api/admin/lms/delivery-status`. 🚧 Once `ws-460` merges the default is `after_response`: the first attempt needs no worker, so rows at `pending` **with `next_retry_at` in the past** mean the scheduler isn't running `lms:deliver-pending` — run it by hand (`--dry-run` first). A HealthStream row that dead-letters instantly with `PAYLOAD_BUILD_ERROR` launched without an `AICC_SID` ([LMS_CONTEXT](../CONTEXT/LMS_CONTEXT.md#healthstream--aicc-ws-460-branch-only)) |
